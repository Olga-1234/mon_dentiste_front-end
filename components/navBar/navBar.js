import Image from "next/image";
import logo from "../../assets/monDentiste.png";
import Button from "../button/Button";
import style from "./navBar.module.css";


const NavBar=({className})=> {
    return (
        <div className={className} >
             <nav className={`navbar fixed navbar-expand-lg  navbar-light`}>
   <div className="container-fluid">
    <button className="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarTogglerDemo01" aria-controls="navbarTogglerDemo01" aria-expanded="false" aria-label="Toggle navigation">
      <span className="navbar-toggler-icon"></span>
    </button>
    <div className="collapse navbar-collapse" id="navbarTogglerDemo01">
      

      <div className="navbar-nav me-auto  mb-lg-0">
        <Image className="" src={logo} alt="logo mon dentiste" width="" height="" />
      </div>
      <div className="d-flex px-5  ">
        
      <ul className={`navbar-nav me-auto mb-lg-0 fw-bold ${style.openSans}`}>
        <li className="nav-item">
          <a className="nav-link px-4" aria-current="page" href="#">Accueil</a>
        </li>
        <li className="nav-item">
          <a className="nav-link px-4" href="#">Trouver un dentiste</a>
        </li>
        <li className="nav-item">
          <a className="nav-link px-4">Articles</a>
        </li>
        <li className="nav-item">
          <a className="nav-link px-4">A propos</a>
        </li>

        <li className="px-4 nav-item">
          <Button text= "connexion"/>
        </li> 
      </ul>

      </div>
    </div>
  </div>
</nav>

        </div>
    )
}

export default NavBar
